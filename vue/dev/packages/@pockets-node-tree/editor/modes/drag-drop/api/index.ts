import type { TreeNodeApi, TreeNode, dropApi } from "@pockets-node-tree"
...
export let createAbstract = (target: TreeNodeApi, selected: TreeNodeApi) => {
    ...
    let targetContainsSelected = () : boolean => {
        ...
        if( target.paths.path.includes(selected.paths.path) ) return true
        return false
    }
    let sameParent = () : boolean => {
        if(!selected.parent || !target.parent) return false;
        return selected.parent.paths.path === target.parent.paths.path
    }
...
export let createDragDropApi = ( 
    ...
    */
    ...
    let dropInside = (dropIndex: number) => {
        return () => {
            if(!target.parent || !target.add.inside || !selected.remove.self) return;
            /**
                1 - Copy node and place in new location
            */
            let node = $object.clone(selected.node)
    ...
    let dropAdjacent = (dropIndex: number) => {
        return () => {
            if(!target.parent || !target.parent.add.inside || !selected.remove.self) return;
            /**
                1 - Copy node and place in new location
            */
            let node = $object.clone(selected.node)
