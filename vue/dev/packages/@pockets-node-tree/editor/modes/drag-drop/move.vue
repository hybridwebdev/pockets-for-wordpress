...
div(
    class='p-2 grid columns-1 gap-2'
)

    div( 
        v-if='!editor.active || selectedActiveSame'
        class='text-center fw-8 grid columns-1 gap-2'
    ) 
        p(
            v-if='!selected'
            class='m-0'
    ...
    div( 
        ...
        p( 
            v-if='!canMoveOrCopy && !selected'
            class='alert alert-danger fw-8 text-center m-0' 
        ) This node cannot be moved

        div( 
            v-if='!selected && canMoveOrCopy'
            class='d-flex gap-2'
        )
            button(
            ...
            button( 
                @click='selectNode' 
                class='btn btn-outline-confirm mx-auto px-8 py-2 fw-8 text-uppercase rounded-0 fw-8'
        ...
        div(
            v-if='selected && !canTargetForDrop' 
        )
            p(
                class='alert alert-danger fw-8 text-center'
            ) This node cannot be targeted 
        div( 
            v-if='selected && canTargetForDrop' 
            class='gap-2 grid columns-1'
        )
            p(
...
export default {
    ...
    setup: () => setup( { removeSelected: true, mode: "move" } ) ,
}
</script>
